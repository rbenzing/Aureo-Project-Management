...
- **Hash passwords ONLY via `App\Utils\PasswordHasher::hash()`.** Never name `PASSWORD_ARGON2ID` directly. It is a compile-time option: the constant is defined on every PHP 8 build (so `defined()` proves nothing), but `password_hash()` throws a `ValueError` — not a warning — when the build lacks libargon2. Hardcoding it made registration, password reset, admin user creation, the CLI installer AND the canonical migration's seed all fatal on such a host; `password_algos()` is the only truthful source and `PasswordHasher` is the one place that consults it. `PASSWORD_PEPPER` in `.env.example` is **read by nothing** — do not describe it as securing anything until some code applies it.
