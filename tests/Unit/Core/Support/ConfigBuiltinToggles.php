 ...
 * global one. Config::loadEnvironment(), Config::initializeSettings() and
 * Database::setDefaultOptions() all call file_exists()/class_exists()
 * unqualified from within App\Core, so once App\Core\file_exists()/
 * class_exists() are declared (see the sibling file, required explicitly by
 * ConfigTest/DatabaseTest), those calls are routed through here instead of
 * touching the real filesystem or class table. This lets the suite exercise
 * Config's defensive "no .env" / "SettingsService unavailable" branches and
 * Database's "no Pdo\\Mysql class" fallback branch without editing src/ or
 * resorting to process isolation.
 *
